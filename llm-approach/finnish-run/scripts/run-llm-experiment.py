...
from templates import template_sentence_with_dep as template
...
def main():
    ...
    if args.run_dir:
        ...
        with open(os.path.join(run_dir, 'md.json'), 'r', encoding='utf-8') as f:
            md = json.load(f)
        model = md['model']
        t8, t11 = md['v2.8'], md['v2.11'] # TODO: versions?
        sents = md['sents']
        ...
        with open(os.path.join(run_dir, 'md.json'), 'w', encoding='utf-8') as f:
            md = {'sentence_count': sentence_count, 'sents': sents, 'model': model, 'now': now, 'run_dir': run_dir, 'v2.8': t8, 'v2.11': t11, 'util_dir': util_dir, 'script_path': script_path, 'api_path': api_path, 'language': language, 'langs_path': langs_path}
            md['prompt'] = template
            if args.note:
                md['note'] = args.note
            json.dump(md, f, ensure_ascii=False, indent=2)

    template = template.format(language=langs[language])

    data_dir = os.path.join(util_dir, 'ud-docs') # TODO: creation of fin json files
    with open(os.path.join(data_dir, 'pos-fi.json'), 'r', encoding='utf-8') as f:
        pos_d = json.load(f)
    with open(os.path.join(data_dir, 'feat-fi.json'), 'r', encoding='utf-8') as f:
        feat_d = json.load(f)
    with open(os.path.join(data_dir, 'dep-fi.json'), 'r', encoding='utf-8') as f:
        dep_d = json.load(f)
    ...
    for run in [v2_8, v2_11]:
        ...
        for sent_id in sents:
            ...
            prompt = get_sentence_prompt(template, table, pos_d, feat_d, dep_d)
