...
def read_conllu(in_filename, gold_filename, morp_column):
    ...
    gold_payload = gold_fin.read().replace('\r\n', '\n')
    gold_fin.close()
    gold_payload = gold_payload.strip().split("\n\n")
    fin = codecs.open(in_filename, "r", encoding="utf-8")
    payload = fin.read().replace('\r\n', '\n')
    fin.close()
    payload = payload.strip().split("\n\n")

    for payload, gold_payload in zip(payload, gold_payload):

        lines = payload.splitlines()
        gold_lines = gold_payload.splitlines()

        body = [line for line in lines if not line.startswith("#") and line != '']
        gold_body = [line for line in gold_lines if not line.startswith("#") and line != '']
